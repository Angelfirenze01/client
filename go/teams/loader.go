...
const (
	LoadTeamFreshness_RANCID LoadTeamFreshness = 0
	LoadTeamFreshness_AGED   LoadTeamFreshness = 1
	LoadTeamFreshness_FRESH  LoadTeamFreshness = 2
)
...
func (l *TeamLoader) load(ctx context.Context, me keybase1.UserVersion, lArg libkb.LoadTeamArg) (res *keybase1.TeamData, err error) {
	...
	var info infoT
	defer l.G().Log.CDebugf(ctx, "TeamLoader#Load -> info:%+v err:%v", info, err)
	...
	// calculate freshness
	freshness := LoadTeamFreshness_RANCID
	if cacheResult != nil {
		// TODO the cache is not always fresh
		freshness = LoadTeamFreshness_FRESH
	...
	cacheTooOld := (freshness != LoadTeamFreshness_FRESH)
	...
	if res == nil {
		return nil, fmt.Errorf("team loader fault: no result")
	}
